...
private:
    ...
    will_erase_from_parent_t do_response()
    {
        ...
        get_request().get_response().then([this](pplx::task<http_response> r_task) {
            ...
            return get_request().content_ready().then([this, response](pplx::task<http_request>) {
...
will_deref_t asio_server_connection::handle_chunked_body(const boost::system::error_code& ec, int toWrite)
{
    ...
    else
    {
        ...
        writebuf.putn_nocopy(buffer_cast<const uint8_t*>(m_request_buf.data()), toWrite)
            .then([this, requestImpl, toWrite](pplx::task<size_t> writeChunkTask) -> will_deref_t {
                try
                {
                    writeChunkTask.get();
                }
...
will_deref_and_erase_t asio_server_connection::async_write(WriteFunc response_func_ptr, const http_response& response)
{
    if (m_ssl_stream)
    {
        boost::asio::async_write(*m_ssl_stream, m_response_buf, [this, response, response_func_ptr](const boost::system::error_code& ec, std::size_t) {
            (this->*response_func_ptr)(response, ec);
        });
    }
    else
    {
        boost::asio::async_write(*m_socket, m_response_buf, [this, response, response_func_ptr](const boost::system::error_code& ec, std::size_t) {
            (this->*response_func_ptr)(response, ec);
        });
    }
    return will_deref_and_erase_t {};
                                                                             ...
                                                                             const boost::system::error_code& ec)
{
    ...
    readbuf.getn(buffer_cast<uint8_t*>(membuf) + chunked_encoding::data_offset, ChunkSize)
        .then([this, membuf, response](pplx::task<size_t> actualSizeTask) -> will_deref_and_erase_t {
            size_t actualSize = 0;
            try
            {
                                                                           ...
                                                                           const boost::system::error_code& ec)
{
    ...
    readbuf.getn(buffer_cast<uint8_t*>(m_response_buf.prepare(readBytes)), readBytes)
        .then([this, response](pplx::task<size_t> actualSizeTask) -> will_deref_and_erase_t {
            size_t actualSize = 0;
            try
            {
                actualSize = actualSizeTask.get();
